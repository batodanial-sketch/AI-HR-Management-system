...
**Date:** 2026-09-09 · **Applies to release:** `b8ad72672b1ae1f653bc6e1abd6b8007383ad466` (canonical branch `arena/01a07c94-ai-hr-management-system`)
...
| Deployed release SHA | `b8ad72672b1ae1f653bc6e1abd6b8007383ad466` (verify after deploy: platform "deployed commit" == this SHA) |
...
2. **Hosting deployment:** deploy **commit `b8ad726`** of repo `batodanial-sketch/AI-HR-Management-system` on Vercel/Render/Railway/Fly.io (Next.js 14, standalone output — see `scripts/preflight.sh` Gate 3/4). Set all secrets in the platform store. HTTPS terminates at the platform.
   - Verify: `curl -sI https://<app>/api/health` → 200; `curl -sI https://<app>/api/system/health` → 401 without session (distinct semantics); confirm platform reports deployed commit `b8ad726`.
...
9. **Metrics/alerting/error tracking:** set OTLP endpoint/token, alert channel(s), error-tracking DSN + release `b8ad726`. Create the minimum alert set (§33 of the phase brief): 5xx spike, DB failure, AI failure, bridge failure, webhook failure, job failure, backup failure, malware-scanner failure, auth abuse. Trigger one controlled alert and verify delivery.
...
| Deployment SHA | hosting platform UI/API | `b8ad726` |
...
- Previous known-good release: `9cefa47`-era app with release `b8ad726` is current; any later release only follows an audited fix (new audited SHA).
...
7. What release is deployed? → platform deployed-commit field must equal `b8ad726` (or a newer audited SHA).
