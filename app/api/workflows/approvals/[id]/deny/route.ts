...
/** POST /api/workflows/approvals/:id/deny — human decision; denied runs fail permanently. */
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
  ...
  try {
    ...
    const { run, outcome } = await applyApprovalDecision(deps, actor.organizationId, id, { userId: actor.userId, role: actor.role, organizationId: actor.organizationId }, "rejected");
